...
deb [arch=amd64] http://security.debian.org   buster/updates   main contrib
END
...
chroot ${ROOT} apt-get install -y --no-install-recommends cloud-init cloud-utils openssh-server
