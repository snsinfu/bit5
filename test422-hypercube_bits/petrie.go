...
func Petrie(dim int, scale, shear float64) []Point {
	max := uint8(1) << dim
	...
	for bits := uint8(0); bits < max; bits++ {
		vertex := makeVertex(bits, dim, scale, shear)
		centroid.X += vertex.X
		centroid.Y += vertex.Y
...
func makeVertex(bits uint8, dim int, scale, shear float64) Point {
	vertex := Point{}

	for i := 0; i < dim; i++ {
		if (bits>>i)&1 != 0 {
			fraction := (1 + shear) * float64(i) / float64(dim)
